...
for (i in seq_len(1E3)){
  sample_data <- aggression_probing_tests %>% 
    ...
    transform_to_sigle_val(column = "actual_fus_numb", mode = 4) %>% # select random fusca number from observed range
...
```
# Difference in the head width of foragers from colonies inhabiting wood ant territory and those living the elsewhere
```{r}
data(head_width)
...
t.test.data <- filter(aggression_probing_tests, territory %in% c("T","B")) %>%
 select(colony,territory) %>% 
  ...
  select(colony,territory,mean) %>% distinct()
x <- filter(t.test.data,territory=="T") %>% select(mean)
y <- filter(t.test.data,territory=="P") %>% select(mean)
cat(sprintf("Mead head width of foragers from wood ant territory: %f", mean(x)))
cat(sprintf("Mead head width of foragers from outside wood ant territory: %f", mean(y)))
t.test(x,y)
```
