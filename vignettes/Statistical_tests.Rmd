...
```
# Introduction
This is an R Markdown document showing how the data was analysed using R environment. All the custom
functions used here along with original data are available through the fusca.defence package
available from the author's GitHub repository.
...
# Make analysis of variance for the first treatments
...
summary(res_2)
```
# Dependence of F. fusca number on the ambient temperature, colony location, experiment duration, and later aggression 
```{r}
library(lme4)
library(lmerTest)
library(DHARMa)
...
# Factors influencing maximial number of F. fusca ants on the arena
```{r eval = FALSE}
set.seed(1001)
...
for (i in seq_len(1E3)){
  ...
  sample_data <- aggression_probing_tests %>%
    ...
    transform_to_single_val(column = "actual_fus_numb", mode = 4) %>% 
    # select random fusca number from observed range
    transform_to_single_val(mode = 4) %>%
...
activity_rufa <- filter(aggression_probing_tests, treatment == "rufa") %>%
  transform_to_single_val(column = "actual_fus_numb",mode = 1) %>%
...
activity_sanguinea <- filter(aggression_probing_tests, treatment == "sanguinea_1") %>%
  transform_to_single_val(column = "actual_fus_numb",mode = 2) %>%
  group_by(colony) %>% summarize(activity = max(actual_fus_numb))
